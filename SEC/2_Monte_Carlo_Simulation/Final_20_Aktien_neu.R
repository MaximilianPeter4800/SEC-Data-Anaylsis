...
# 4️⃣ Richtige Loading-Reihenfolge: pro Jahr 1 Zeile, 12 Monate lang wiederholen
# z.B. n_years = 10  →  Zeile 1 = Jahr 1, Zeile 2 = Jahr 2, etc.
loadings_for_path <- sim_results[1:n_years, ] %>%
  slice(rep(1:n(), each = 12))  # 12 Monate pro Jahr

# 5️⃣ Korrekte Length prüfen
stopifnot(nrow(loadings_for_path) == n_months)

# 6️⃣ Berechne Portfolio-Return
...
# 7️⃣ Total Return & CAGR
total_return <- prod(1 + monthly_return) - 1
cagr <- (1 + total_return)^(1/10) - 1
...
simulated_cagr_D <- (1 + simulated_total_returns_D)^(1/10) - 1
median(simulated_cagr)

# ======================================================================
#  ▸ CPI-Inflation 2013–2022 aus FRED (Monthly US CPI, not seasonally adjusted)
# ======================================================================
getSymbols("CPIAUCNS", src = "FRED")  # US Consumer Price Index, All Urban Consumers

cpi <- CPIAUCNS["2013/2022"]

# Jährliche Inflationsraten berechnen (Dezember-zu-Dezember)
cpi_df <- data.frame(date = index(cpi), value = as.numeric(cpi)) %>%
  mutate(year = year(date)) %>%
  group_by(year) %>%
  filter(month(date) == 12) %>%
  summarise(cpi_dec = first(value)) %>%
  mutate(inflation_rate = (cpi_dec / lag(cpi_dec) - 1)) %>%
  filter(!is.na(inflation_rate))

# Ø jährliche Inflation (arithmetisch oder geometrisch)
...
ggplot(df_returns, aes(x = real_return)) +
  ...
  labs(
    ...
    subtitle = "20-stock portfolio, 10-year horizon, incl. trading costs, taxes, inflation",
